...
function AllDiliveredProducts() {
  ...
  useEffect(() => {
    axios
      .get(`${backendLink}/orders/diliveredProduct/getByName?name=${name}&page=${page}`)
      .then((res) => {
        setData(res.data);
      })
  ...
  const handleOrderSerachByName = () => {
    axios
      .get(
        `${backendLink}/orders/admin/deliverd&page=${page}`
      )
      .then((res) => {
        setData(res.data);
      })
...
export default AllDiliveredProducts